  ...
  const fetchDashboardData = async () => {
    try {
      const [statsResponse, ordersResponse] = await Promise.all([
        ...
        // The API automatically filters orders for delivery partners based on their role
        axios.get('/api/orders?limit=10&sortBy=orderPlacedAt&sortOrder=desc')
      ]);

      setStats(statsResponse.data.data);
      
      // All orders returned should be assigned to current delivery partner
      const userOrders = ordersResponse.data.data.orders || [];
      console.log('🚚 API Response - All orders for delivery partner:', userOrders);
      console.log('🚚 Current user ID:', user._id);
      
      // Backend should only return orders where deliveryPartner matches current user
      // Filter out completed/cancelled orders for the "assigned orders" section
      const activeAssignedOrders = userOrders.filter(order => 
        !['DELIVERED', 'CANCELLED'].includes(order.status)
      );
      
      console.log('🚚 Active assigned orders for dashboard:', activeAssignedOrders);
      setAssignedOrders(activeAssignedOrders);
    } catch (error) {
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-white">{stats.totalDeliveries || 0}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-white">{stats.pendingPickups || 0}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-white">{stats.activeDeliveries || 0}</p>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          <div className="flex items-center">
            ...
            <div className="ml-4">
              ...
              <p className="text-2xl font-bold text-gray-900 dark:text-white">{stats.completedToday || 0}</p>
            </div>
          </div>
        </div>
      ...
      <div className="mb-8">
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700 p-6">
          <div className="flex items-center justify-between">
            ...
            <Link 
              to="/my-stats" 
              className="bg-gradient-to-r from-primary-600 to-accent-600 hover:from-primary-700 hover:to-accent-700 text-white px-4 py-2 rounded-lg font-medium transition-all duration-200 inline-flex items-center"
            >
        ...
        <div className="lg:col-span-2">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
            <div className="p-6 border-b border-gray-200 dark:border-gray-700">
              <div className="flex items-center justify-between">
                <h2 className="text-xl font-semibold text-gray-900 dark:text-white">Your Assigned Orders</h2>
            ...
            <div className="divide-y divide-gray-200 dark:divide-gray-700">
              {assignedOrders.length > 0 ? (
                  ...
                  <div key={order._id} className="p-6">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600 dark:text-gray-400">{order.customerAddress?.street}, {order.customerAddress?.city} - {order.customerAddress?.pincode}</p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                      <div className="flex items-center space-x-4">
                        <span className="text-sm text-gray-600 dark:text-gray-400">
                          Total: <span className="font-medium text-gray-900 dark:text-white">{formatCurrency(order.totalAmount)}</span>
                        </span>
                        <span className="text-sm text-gray-600 dark:text-gray-400">
                          Phone: <span className="font-medium text-gray-900 dark:text-white">{order.customerPhone}</span>
                        </span>
                      </div>
                      <div className="flex space-x-2">
                        {getNextStatus(order.status) && (
                          <button
                            ...
                            className="bg-gradient-to-r from-primary-600 to-accent-600 hover:from-primary-700 hover:to-accent-700 text-white px-3 py-1.5 rounded-lg font-medium transition-all duration-200 text-sm"
                          >
                        ...
                        <Link
                          to={`/orders/${order._id}`}
                          className="bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 px-3 py-1.5 rounded-lg font-medium transition-all duration-200 text-sm"
                        >
                          View Details
                        </Link>
                      </div>
                ...
                <div className="text-center py-12">
                  ...
                  <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">No active orders assigned</h3>
                  <p className="text-gray-500 dark:text-gray-400 mb-2">
                    {isAvailable 
                      ? 'Restaurant managers will assign orders to you when they become ready for delivery' 
                      : 'Turn on availability to receive order assignments'
                    }
                  </p>
                  <p className="text-xs text-gray-400 dark:text-gray-500">
                    Orders in PREP, READY, PICKED, or ON_ROUTE status will appear here
                  </p>
                </div>
              )}
            </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-sm border border-gray-200 dark:border-gray-700">
            ...
            <div className="p-6 space-y-6">
              ...
              <div className="pt-2">
                <Link 
                  to="/my-stats" 
                  className="w-full bg-gray-100 dark:bg-gray-700 hover:bg-gray-200 dark:hover:bg-gray-600 text-gray-700 dark:text-gray-300 px-4 py-2 rounded-lg font-medium transition-all duration-200 text-center block"
                >
